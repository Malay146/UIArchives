...
import { useProductSearch } from "@/lib/search/useProductSearch";
...
const FilterSearch = forwardRef<HTMLDivElement>((props, ref) => {
  ...
  const { query, setQuery, results, isSearching } = useProductSearch(cardData);

  // Filter search results by the selected tag locally
  const finalCards = useMemo(() => {
    let filtered = results;
    if (selectedTag !== "All") {
      filtered = filtered.filter((card) => {
        if (Array.isArray(card.tag)) return card.tag.includes(selectedTag);
  ...
  return (
    <>
      ...
      {/* Cards */}
      <CardGrid
        cards={finalCards as import("./ui-card").UiCardData[]}
